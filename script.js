...
function renderMusicLibrary(songs, remember = true) {
  ...
  const searchFilter = normalizeSearchText($("#musicSearchFilter")?.value);
  const orderedSongs = [...latestSongs].sort((left, right) => (
    new Date(right.criado_em).getTime() - new Date(left.criado_em).getTime()
  ));
  const visibleSongs = orderedSongs.filter((song) => (
    (authorFilter === "todos" || song.autor === authorFilter)
    && (!searchFilter || normalizeSearchText(`${song.titulo} ${song.artista}`).includes(searchFilter))
  ));
  ...
  if (!visibleSongs.length) {
    ...
    empty.textContent = searchFilter
      ? "Nenhuma música combina com essa busca."
      : `${authorFilter} ainda não adicionou uma música à nossa playlist.`;
    list.append(empty);
    return;
  }
...
async function loadMusicLibrary() {
  try {
    const response = await fetch(
      `${SUPABASE.url}/rest/v1/musicas?select=id,titulo,artista,youtube_video_id,autor,criado_em&order=criado_em.desc`,
...
function renderFuturePlans(plans, remember = true) {
  ...
  const visiblePlans = latestFuturePlans
    .filter((plan) => (
  ...
  if (statusFilter === "todos") {
    list.append(createFuturePlanGroup("Nossos planos mais recentes", pagedPlans.items, false));
...
async function loadFuturePlans() {
  try {
    const response = await fetch(
      `${SUPABASE.url}/rest/v1/planos_futuros?select=id,titulo,detalhes,categoria,autor,concluido,concluido_em,criado_em&order=criado_em.desc`,
      { headers: { apikey: SUPABASE.publishableKey } }
    );
...
function setupContentFilters() {
  ...
  const musicSearch = $("#musicSearchFilter");
  musicSearch.addEventListener("input", () => {
    resetCollectionPage("music");
    renderMusicLibrary(latestSongs, false);
  });
